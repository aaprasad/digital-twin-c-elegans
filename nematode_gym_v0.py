...
def make_nematode(joint_range, body_len, muscle_len, arrangement=None, camera_pos=None):
    """ create nematode env """
    # generate xml str
    xml_folder = 'src/envs/mujoco/assets/'
    xml_str = nematode(joint_range=joint_range, body_len=body_len, muscle_len=muscle_len, xml_file=os.path.join(xml_folder, 'swimmer.xml'), arrangement=arrangement, camera_pos=camera_pos)
...
def test_random():
    """ take random actions """
    # body_len >= 0.2
    env = make_nematode(joint_range='-100 100', body_len=0.26, muscle_len=0.26, arrangement=None, camera_pos='0 -5 5')
    # record video
