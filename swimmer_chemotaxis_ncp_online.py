...
def online_test(
    ...
    model = prepare_model(
        model_name, model_path=os.path.join(model_dir, 'model.pt'),
        **{'units': units, 'output_dim': output_dim, 'in_features': in_features}
    )
    ...
    writer.add_hparams(
        {
            ...
            'units': units, 'output_dim': output_dim, 'in_features': in_features, 'model_dir': model_dir, 'data_size': data_size
        },
...
if __name__ == '__main__':
    online_test(data_size=120, model_dir=None, data_name='ncp.pt', model_name='fully_connected_model')
