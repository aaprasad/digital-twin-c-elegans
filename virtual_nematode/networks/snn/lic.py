...
class LIC51(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC51, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        ...
        self.w_p_n = torch.nn.Parameter(w_p_n, requires_grad=False)  # (n, )
        self.output_index = torch.nn.Parameter(output_index, requires_grad=False)  # (n, ), bool
        self.delta_state_func = Sigmoid(a=-1.5, b=1.5)  # delta state ~(-1.5,1.5), unit 0.1 V
        self.activation_func = Activation(k=37.5, b=9.)
        self.s = s  # sensory size
        self.w_s = torch.nn.Parameter(torch.ones(s))  # (3, )
        self.w_s_mask = torch.nn.Parameter(w_s_mask, requires_grad=False)  # (2, ), long
    ...
    def forward(self, state, activation, stimuli):
        ...
        for i in range(self.steps):
            ...
            total_input = synapse_input + gap_input + external_input
            # cell state and activation
            delta_state = dt_tau * total_input
            delta_state = self.delta_state_func(delta_state)
            state = (1 - dt_tau) * state + delta_state
            activation = self.activation_func(state)
        # muscle output
        action = activation[:, self.output_index]
        return state, activation, action
...
class LIC61(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC61, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC62(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC62, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC70(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC70, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        ...
        # self.input_func = Sigmoid(a=-50, b=50)
        # self.input_func = Sigmoid(a=-100, b=100)
        self.activation_func = Activation(k=37.5, b=9.)
        self.s = s  # sensory size
        self.w_s = torch.nn.Parameter(torch.ones(s))  # (3, )
        self.w_s_mask = torch.nn.Parameter(w_s_mask, requires_grad=False)  # (2, ), long
...
class LIC71(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC71, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC72(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC72, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC73(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC73, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC80(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC80, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC81(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC81, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
...
class LIC82(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC82, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        ...
        self.w_p_n = torch.nn.Parameter(w_p_n, requires_grad=False)  # (n, )
        self.output_index = torch.nn.Parameter(output_index, requires_grad=False)  # (n, ), bool
        self.state_func = Tanh(a=-0.8, b=0.35)
        self.activation_func = Activation(k=37.5, b=9.)
        self.s = s  # sensory size
        self.w_s = torch.nn.Parameter(torch.ones(s))  # (3, )
        self.w_s_mask = torch.nn.Parameter(w_s_mask, requires_grad=False)  # (2, ), long
...
class LIC90(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC90, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        self.p = p
        tau = torch.zeros(n).uniform_(0.008, 0.2)
        tau_func = SigmoidFunc(a=0.008, b=0.2)
        tau = tau_func.inverse(tau)
        self.tau = torch.nn.Parameter(tau)  # (n, )
        self.tau_func = tau_func
        self.bias = torch.nn.Parameter(-0.35 + torch.zeros(n).normal_(0, 0.01))  # (n, )
        ...
        e_c = torch.zeros((n, n)).normal_(mean=0, std=0.01)
        e_c = (-0.24 + e_c) * w_c_mask[0]
        e_c_func = SigmoidFunc(a=-0.5, b=0.05)
        e_c = e_c_func.inverse(e_c)
        self.e_c = torch.nn.Parameter(e_c)  # (n, n)
        self.e_c_func = e_c_func
        self.e_c_mask = torch.nn.Parameter(w_c_mask, requires_grad=False)  # (3, n, n), bool
...
class LIC91(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC91, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
    ...
    def forward(self, state, activation, stimuli):
        ...
        w_g = (w_g.tril() + w_g.tril(diagonal=-1).T) * self.w_g_mask
        # external input
        external_input = self._external_input(stimuli)
        # dt / tau
        tau = self.tau_func(self.tau)
        dt_tau = self.dt / self.steps / tau
        for i in range(self.steps):
            ...
            total_input = self.input_func(total_input)
            # cell state and activation
            state = (1 - dt_tau) * state + dt_tau * (total_input + self.bias)
            activation = self.activation_func(state)
        # muscle output
        action = activation[:, self.output_index]
        return state, activation, action
...
class LIC92(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC92, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        ...
        self.w_p_n = torch.nn.Parameter(w_p_n, requires_grad=False)  # (n, )
        self.output_index = torch.nn.Parameter(output_index, requires_grad=False)  # (n, ), bool
        self.state_func = SigmoidFunc(a=-0.8, b=0.35)
        self.activation_func = Activation(k=37.5, b=9.)
        self.s = s  # sensory size
        self.w_s = torch.nn.Parameter(torch.ones(s))  # (3, )
        self.w_s_mask = torch.nn.Parameter(w_s_mask, requires_grad=False)  # (2, ), long
...
class LIC93(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index, s, w_s_mask):
        super(LIC93, self).__init__()
        self.dt = dt
        self.steps = steps
        self.n = n
        self.m = m
        ...
        self.w_p_n = torch.nn.Parameter(w_p_n, requires_grad=False)  # (n, )
        self.output_index = torch.nn.Parameter(output_index, requires_grad=False)  # (n, ), bool
        self.state_func = torch.nn.Hardtanh(min_val=-0.8, max_val=0.35)
        self.activation_func = Activation(k=37.5, b=9.)
        self.s = s  # sensory size
        self.w_s = torch.nn.Parameter(torch.ones(s))  # (3, )
        self.w_s_mask = torch.nn.Parameter(w_s_mask, requires_grad=False)  # (2, ), long
