...
class LeakyIntegratorConductanceBasedUnrestrained(torch.nn.Module):
    ...
    def init_state(self):
        ...
        #bias = self.bias.clone().detach()
        #state = bias
        state = torch.full((self.n, ), fill_value=self.bias)
        activation = self.activation_func(state)
        return state, activation  # (n, ), (n, )
...
class LeakyIntegratorConductanceBasedRestrained(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index):
        super(LeakyIntegratorConductanceBasedRestrained, self).__init__()
        self.dt = dt  # 0.04
        self.steps = steps  # 5
        self.n = n
        self.m = m
    ...
    def init_state(self):
        ...
        #bias = self.bias.clone().detach()
        #state = bias
        bias = torch.full((self.n, ), fill_value=self.bias)
        state = self.state_func(bias)
        activation = self.activation_func(state)
        activation = (activation - self.activation_scaling[0]) / (self.activation_scaling[1] - self.activation_scaling[0])
        return state, activation  # (n, ), (n, )
...
class LeakyIntegratorConductanceBasedRestrained1(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index):
        super(LeakyIntegratorConductanceBasedRestrained1, self).__init__()
        self.dt = dt  # 0.04
        self.steps = steps  # 5
        self.n = n
        self.m = m
...
class LeakyIntegratorConductanceBasedRestrained2(torch.nn.Module):
    def __init__(self, dt, steps, n, m, p, w_c_mask, w_g_mask, w_p_mask, output_index):
        super(LeakyIntegratorConductanceBasedRestrained2, self).__init__()
        self.dt = dt  # 0.04
        self.steps = steps  # 5
        self.n = n
        self.m = m
