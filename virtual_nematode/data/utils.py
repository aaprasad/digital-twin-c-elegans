...
from virtual_nematode.data.split import SplitDataset, SlidingWindowDataset, SlidingWindowSampleDataset
...
def preprocess_and_split_slide_train_dataset(load_name, save_names, lengths, x_index, seq_len, seq_amount, stride, seed):
    # load
    dataset = torch.load(os.path.join('data', load_name))
    print('load', dataset.tensors[0].shape, dataset.tensors[1].shape)
    # subset
...
def preprocess_and_split_slide_sample_dataset(load_name, save_names, lengths, x_index, seq_len, seq_range, seq_amount, stride, seed):
    # load
    dataset = torch.load(os.path.join('data', load_name))
    print('load', dataset.tensors[0].shape, dataset.tensors[1].shape)
    # subset
    ...
    print('test', test_data.tensors[0].shape, test_data.tensors[1].shape)
    # split sequence
    train_data = SlidingWindowSampleDataset(train_data, seq_len, seq_range, seq_amount, stride, seed)
    print('split sequence train', train_data.tensors[0].shape, train_data.tensors[1].shape)
    eval_data = SlidingWindowSampleDataset(eval_data, seq_len, seq_range, seq_amount, stride, seed)
    print('split sequence eval', eval_data.tensors[0].shape, eval_data.tensors[1].shape)
    test_data = SlidingWindowSampleDataset(test_data, seq_len, seq_range, seq_amount, stride, seed)
    print('split sequence test', test_data.tensors[0].shape, test_data.tensors[1].shape)
    # save
    torch.save(train_data, os.path.join('data', save_names[0]))
    torch.save(eval_data, os.path.join('data', save_names[1]))
